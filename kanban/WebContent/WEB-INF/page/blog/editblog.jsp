...
<html lang="zh">
  ...
  <body>
   ...
   <div class="container">
     ...
     <s:form name="editblog" action="saveblog" method="POST" theme="simple" role="form" class="form-horizontal">
        <s:hidden name="blog.id" value="%{blog.id}"/>
        <s:hidden name="blog.logo.id" value="%{blog.logo.id}"/>
        ...
        <div class="form-group logo">
            <img src='<s:property value="blog.logo.path"/>' class="original_img"/><img id="upload_image" src="${pageContext.request.contextPath}/images/upload.png" /><input type="file" name="image" id="fileupload_input" style="display:none;"/>		
        </div>      
        <div class="form-group">
	      <div class="checkbox">
	        <label>
	    	...
	    	$("#upload_image").live('click',function(){
	           ...
	           //formData:{param1:"p1",param2:"p2"},//如果需要额外添加参数可以在这里添加  
	        	...
	        	/* if($("#photo").children("img").length)
	           	{
	           	$("#photo img").attr("src",result.result.updatedLogoPath);
	           	}
	           else{
	           	$("#upload_image").css("margin-left","90px");
	           	$("#upload_image").css("margin-top","50px");
	           	
	           }
	           */
	           $("#saveblog_blog_logo_id").attr("value",result.result.attachmentId);
	           $(".original_img").attr("src",result.result.uploadedFilePath);
	           }   
	       });
